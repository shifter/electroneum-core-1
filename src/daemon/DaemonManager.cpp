...
bool DaemonManager::start(const QString &flags, bool testnet, const QString &dataDir)
{
    ...
    qDebug() << "starting daemon " + m_monerod;
    ...
    // Start daemon with console
    //bool started = m_daemon->startDetached(m_monerod, arguments);
    // Start daemon without console
    m_daemon->start(m_monerod, arguments);
    bool started =  false;
    if(m_daemon->state() == QProcess::Starting || m_daemon->state() == QProcess::Running){
        started = true;
    }
...
{ 
    ...
    qDebug() << status;
    // `./electroneumd status` returns BUSY when syncing.
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to daemond
#ifdef Q_OS_WIN
