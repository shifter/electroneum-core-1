...
for PR in $(git log --format=%B -n 1 | grep -io "requires #[0-9]*" | sed 's/[^0-9]*//g'); do
    echo "Merging Electroneum push request #$PR"
...
# Build libwallet if no previous version file exists
    ...
    echo "electroneum/version.h not found - Building libwallet"
    BUILD_LIBWALLET=true
...
echo "cleaning up existing Electroneum build dir, libs and includes"
#rm -fr $MONERO_DIR/build
